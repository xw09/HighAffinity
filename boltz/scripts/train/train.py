...
@dataclass
class TrainConfig:#数据类，包含了模型、数据、训练、日志等配置
    """Train configuration.
...
def train(raw_config: str, args: list[str]) -> None:  # noqa: C901, PLR0912, PLR0915
    ...
    """
    # Load the configuration加载初始的 YAML 配置
    raw_config = omegaconf.OmegaConf.load(raw_config)

    # Apply input arguments解析命令行参数，并与原始配置合并
    ...
    cfg = TrainConfig(**cfg)#数据类，包含了模型、数据、训练、日志等配置

    # Set matmul precision设置浮点精度
    ...
    # Create objects处理训练集的输入
    data_config = DataConfig(**cfg.data)
    data_module = BoltzTrainingDataModule(data_config)#数据加载、预处理和划分
    ...
    # Create checkpoint callback定义了训练过程中如何保存模型，包括保存验证指标 val/lddt 最优模型。
    callbacks = []
    dirpath = cfg.output
    ...
    # Set up trainer初始化 Trainer 并启动训练
    strategy = "auto"
